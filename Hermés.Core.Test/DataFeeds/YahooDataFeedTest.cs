...
{
    [TestClass]
    public class YahooDataFeedTest
    {
        public void YahooDataFeed_HeaderParsing()
        {
            var fileName = System.IO.Path.GetTempPath() + Guid.NewGuid().ToString() + "_yahoo.csv";
            using (var inFileWriter = new StreamWriter(fileName))
            {
                inFileWriter.Write(@"
EXCHANGE%3DINDEXDJX
